...
func determine_star_system(star_type: String, star_tier: int) -> String:
	...
	match star_type:
		...
		"White":
			...
			var trinary_prob = 0
			if star_tier >= 4:
				log10(star_tier - 3) * 50   # Proper ternary syntax in GDScript
			system_type = choose_with_probability({"Singular": max(100 - binary_prob - trinary_prob), "Binary": binary_prob, "Trinary": trinary_prob})
		"Blue":
			...
			var trinary_prob = max(100 - singular_prob - binary_prob)  # Increases with higher tiers
			system_type = choose_with_probability({"Singular": singular_prob, "Binary": binary_prob, "Trinary": trinary_prob})

	return system_type

func choose_with_probability(options: Dictionary):
	var total_prob = 0.0
